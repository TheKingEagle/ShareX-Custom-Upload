<?php
    ...
    if($config['enable_delete'] && isset($_GET['action']) && 'delete' === $_GET['action']) {
        if(file_exists($_GET['filename'])) {
            ...
            $_SESSION['message'] = 'You have successfully deleted <strong>'.basename($_GET['filename']).'</strong>';
            die(header('Location: /'));
        }else{
...
<html>
	...
	<body>
		<div class="container">
			...
			<?php
                if(auth_user(false)){
                    $files1 = preg_grep('/^([^.])/', scandir(dirname(dirname(__FILE__))));
                    ...
                    unset($files1[$key]);
						...
						<tr>
							<td>
								...
								<?php if($config['enable_tooltip'] && isImage(finfo_file($finfo,$absfile))){?> 
							...
							<td>
								<?php echo filesize($absfile); ?>
							</td>
							<td>
								<?php echo date('d M Y H:i',fileatime($absfile)); ?>
							</td>
							<td>
							...
							<?php if($config['enable_delete']){?>   
								<td>
									<a href="index.php?action=delete&filename=<?php echo $absfile; ?>" class="btn btn-danger" onclick="return confirm('Are you sure you want to permanently delete this file (<?php echo $file; ?>) ?');">
										delete file
									</a>
								</td>
							<?php }?> 
